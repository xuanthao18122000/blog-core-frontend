  ...
  <DashboardLayout>
    <main class="transaction">
      <div class="page-heading">
        <h3>Blog Core</h3>
        <p class="text-subtitle text-muted">You have {{total}} Blog Core.</p>
      </div>

      <div class="page-content">
        <div class="card">
          <div class="card-header pb-0">
            <h4 class="card-title">Table list Blog</h4>
...
import DashboardLayout from '@/layouts/LayoutAdmin.vue';
import PaginationView from '@/components/PaginationView.vue';
import { ref, reactive, onMounted } from 'vue';
import { listPost } from '@/api/posts';
...
async function getListTransaction() {
  ...
  const response  = await listPost(filter)
  ...
  if(response.status) {
    ...
    toast.error(getErrorMsg(data.msg))
  }
}
